...
Searches for the Arithma-App directory as a sibling of the library checkout
(developer path). Falls back to ~/.arithma-app, cloning from GitHub at the
...
_APP_REPO   = "https://github.com/andrewkwatts-maker/Arithma-App.git"
_APP_FOLDER = "Arithma-App"
_APP_MODULE = "arithma_app"
_LIB_FILE   = Path(__file__)


def _library_version() -> str:
    try:
        return _pkg_version("arithma")
    except Exception:
        return ""
...
def _find_or_clone() -> Path:
    ...
    home_dir = Path.home() / ".arithma-app"
    if home_dir.is_dir():
        return home_dir
